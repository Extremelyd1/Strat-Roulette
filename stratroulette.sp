...
public Action:cmd_srtest(client, args) {
	...
	if (args >= 2) {
		GetCmdArg(2, model2, sizeof(model2));
	}

	if (StrEqual(model, "floor")) {
		float clientPos[3];
		GetEntPropVector(client, Prop_Send, "m_vecOrigin", clientPos);

		int scaffolding = CreateEntityByName("prop_dynamic");

		SetEntityModel(scaffolding, "models/props/de_vertigo/scaffolding_walkway_03.mdl");
		DispatchKeyValue(scaffolding, "Solid", "6");
		TeleportEntity(scaffolding, clientPos, NULL_VECTOR, NULL_VECTOR);
		DispatchSpawn(scaffolding);
		AcceptEntityInput(scaffolding, "EnableCollision");

		return Plugin_Continue;
	}

	if (StrEqual(model, "smoke")) {
		if (lastSmokeEntity != -1) {
			AcceptEntityInput(lastSmokeEntity, "kill");
			lastSmokeEntity = -1;
		}
		...
		DispatchKeyValue(smokeEntity, "effect_name", model2);
		DispatchSpawn(smokeEntity);
		TeleportEntity(smokeEntity, clientPos, NULL_VECTOR, NULL_VECTOR);
		ActivateEntity(smokeEntity);
		AcceptEntityInput(smokeEntity, "Start");

		lastSmokeEntity = smokeEntity;

		CreateTimer(5.0, SmokeRemoveTimer);

		/* int smokeEntity = CreateEntityByName("smokegrenade_projectile");
		if (smokeEntity != -1) {
			float clientPos[3];
			GetEntPropVector(client, Prop_Send, "m_vecOrigin", clientPos);
			...
			SetEntProp(smokeEntity, Prop_Send, "m_nSmokeEffectTickBegin", GetGameTickCount());

			SetEntityMoveType(smokeEntity, MOVETYPE_NONE);
		} */
	...
	if (args < 2) {
		int wood_wall = CreateEntityByName("prop_dynamic");

		SetEntityModel(wood_wall, model);

		DispatchKeyValue(wood_wall, "Solid", "6");

		float clientPos[3];
		GetEntPropVector(client, Prop_Send, "m_vecOrigin", clientPos);

		TeleportEntity(wood_wall, clientPos, NULL_VECTOR, NULL_VECTOR);

		DispatchSpawn(wood_wall);

		AcceptEntityInput(wood_wall, "EnableCollision");

		float vecMin[3];
		GetEntPropVector(wood_wall, Prop_Send, "m_vecMins", vecMin);

		float vecMax[3];
		GetEntPropVector(wood_wall, Prop_Send, "m_vecMaxs", vecMin);

		PrintToServer("wood_wall vecMin: %f, %f, %f", vecMin[0], vecMin[1], vecMin[2]);
		PrintToServer("wood_wall vecMax: %f, %f, %f", vecMax[0], vecMax[1], vecMax[2]);
	} else if (args < 3) {
		float clientPos[3];
		GetEntPropVector(client, Prop_Send, "m_vecOrigin", clientPos);
		...
		for (int y = 0; y < 5; y++) {
			for (int z = 0; z < 5; z++) {
				float propPos[3];
				propPos[0] = clientPos[0];
				propPos[1] = clientPos[1] + y * 56.0;
				propPos[2] = clientPos[2] + z * 112.0;
				...
				if (model1) {
					...
					SetEntityModel(wood_wall, model2);
				}
				...
				float angle[3];
				GetEntPropVector(wood_wall, Prop_Send, "m_angRotation", angle);

				angle[0] += 180.0;

				TeleportEntity(wood_wall, propPos, angle, NULL_VECTOR);
		...
		for (int z = 0; z < 5; z++) {
			for (int y = 0; y < 5; y++) {
				float propPos[3];
				propPos[0] = clientPos[0];
				propPos[1] = clientPos[1] + y * 200.0;
				propPos[2] = clientPos[2] + z * 150.0;
...
public SetServerConvars() {
	...
	if (!pugSetupLoaded) {
		SetConVarInt(mp_freezetime, 0, true, false);
	}
}
